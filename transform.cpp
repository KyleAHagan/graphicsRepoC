...
{
    const glm::vec3 V = glm::normalize(Center - Eye);
    const glm::vec3 A = glm::normalize(glm::cross(V, Up));
    const glm::vec3 B = glm::cross(A, V);

    return glm::transpose(glm::mat4(A.x, A.y, A.z, -(glm::dot(A, Eye)),
        B.x, B.y, B.z, -(glm::dot(B, Eye)),
